...
class APISpecsView(MethodView):
    ...
    def get(self):
        ...
        try:
            # print(self.loader())
            loader = self.loader()
            for spec in loader.keys():
                if type(loader[spec]) == dict:
                    loader[spec] = dict(loader[spec])
            ss = json.dumps(loader)
            return jsonify(loader)
        except Exception as e:
            traceback.print_exc()
            print(e)
            import pickle
            with open('test', 'wb') as load:
                load.write(pickle.dumps(self.loader()))
            # with open('apispec_loader', 'w') as loader:
            #     loader.write(self.loader())
            return Response(
                json.dumps(self.loader()),
...
class Swagger(object):
    ...
    def get_url_mappings(self, rule_filter=None):
        """
        Returns all werkzeug rulesgunicorn==20.0.4
        """
